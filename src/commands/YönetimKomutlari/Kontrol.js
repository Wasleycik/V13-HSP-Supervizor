...
module.exports = {
    ...
    return }
       ...
       const etkinlik = await client.guilds.cache.get(ayar.GuildID).roles.cache.find(x => x.name.includes(ayar.etkinlik)).id
       const cekilis = await client.guilds.cache.get(ayar.GuildID).roles.cache.find(x => x.name.includes(ayar.cekilis)).id
